...
import 'package:movies_app/controllers/search_controller.dart' as searchImport;
        ...
        child: Column(
          children: [
            ...
            SearchBox(
              onSumbit: () {
                String search =
                    Get.find<searchImport.SearchController>().searchController.text;
                Get.find<searchImport.SearchController>().searchController.text = '';
                Get.find<searchImport.SearchController>().search(search);
            ...
            Obx(
              (() => Get.find<searchImport.SearchController>().isLoading.value
                  ? const CircularProgressIndicator()
                  : Get.find<searchImport.SearchController>().foundedMovies.isEmpty
                      ? SizedBox(
                          width: Get.width / 1.5,
                          child: Column(
                            children: [
                      ...
                      : ListView.separated(
                          itemCount:
                              Get.find<searchImport.SearchController>().foundedMovies.length,
                          shrinkWrap: true,
                          physics: const NeverScrollableScrollPhysics(),
                          separatorBuilder: (_, __) =>
                              const SizedBox(height: 24),
                          itemBuilder: (_, index) {
                            Movie movie = Get.find<searchImport.SearchController>()
                                .foundedMovies[index];
